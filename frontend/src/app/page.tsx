      ...
      onLogs: logs => {
        logs.forEach(log => {
          ...
          setAgents(prev => {
            ...
            return [{ id: `${newAgent.substring(0, 6)}...${newAgent.substring(38)} (New Agent)`, balance: '0.00 XOKB' }, ...prev];
          });
        });
      }
    });
          ...
          {account ? (
            <div className="flex items-center space-x-3">
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded-lg text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-900/20"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="col-span-1 border border-gray-800 rounded-2xl bg-gray-900/50 p-6 shadow-xl shadow-black/20 backdrop-blur-sm">
          <h2 className="text-xl font-semibold mb-6 flex items-center text-gray-100">
            <span className="w-2.5 h-2.5 rounded-full bg-blue-500 mr-3 shadow-[0_0_8px_rgba(59,130,246,0.8)]"></span>
            Registered Agents
          </h2>
          <div className="space-y-4">
            {agents.map((agent, idx) => (
              <div key={idx} className="flex justify-between items-center p-3.5 rounded-xl bg-gray-800/40 border border-gray-700/50 hover:bg-gray-800/60 transition-colors">
        ...
        <div className="col-span-1 md:col-span-2 border border-gray-800 rounded-2xl bg-gray-900/50 p-6 shadow-xl shadow-black/20 backdrop-blur-sm">
          <h2 className="text-xl font-semibold mb-6 flex items-center text-gray-100">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 mr-3 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse"></span>
          ...
          <div className="space-y-3 overflow-hidden">
            {events.length === 0 ? (
              ...
              events.map((ev, idx) => (
                <div key={idx} className="flex justify-between items-center p-4 rounded-xl bg-gray-800/30 border border-gray-700/50 hover:border-gray-600 transition-all group">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <span className="text-sm font-bold text-emerald-400 bg-emerald-950/40 px-3 py-1 rounded-lg border border-emerald-900/50 shadow-sm shadow-emerald-900/20">+{ev.amount} XOKB</span>
                  </div>
                </div>
              ))
            )}
